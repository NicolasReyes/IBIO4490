...
data_path = '../images/'; %change if you want to work with a network copy
...
lambda = 0.00001;
feats  = [features_pos; features_neg]; %Colocamos los features_pos y features_neg. (16851x2511)
labels  = [ones(size(features_pos,1),1); -1*ones(size(features_neg,1),1)]; %Labels correspondientes a los features_pos y features_neg

[w, b] = vl_svmtrain(feats', labels, lambda); %Funcion para entrenar el Support Vector Machine. (W, Weight Vector. B, offset). Score W'*X(:,i)+B
save('w.mat','w')
save('b.mat','b')
...
legend('Face Confiance','NO Face Confiance','')
hold off;
