...
function [bboxes, confidences, image_ids,random_number] = .... 
    run_detector_demo(test_scn_path, w, b, feature_params)
...
test_scenes = dir( fullfile( test_scn_path, '*.png' ));
...
scales = [1, 0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.5, 0.4, 0.25, 0.15, 0.1, 0.07];
%scales = [1, 0.5, 0.4, 0.1];

%Caso4:threshold = 0.85;
%Caso5:threshold = 0.80;
%Caso6:threshold = 0.95;
%Caso7:threshold = 0.7;
%Caso8:threshold = 0.92;
%Caso9: threshold = 0.9 y neg=20000
%Caso10: threshold = 0.8 y neg=10000 y lamda=0.00001 
%Caso11: threshold = 0.85 y neg=10000 y lamda=0.00001 
%Caso12: threshold = 0.9 y neg=10000 y lamda=0.00001 
%Caso13: threshold = 0.93 y neg=10000 y lamda=0.00001 
%Caso14: threshold = 0.95 y neg=10000 y lamda=0.00001 
...
random_number = randi([1,length(test_scenes)]);
%for i = 1:length(test_scenes)
      
    fprintf('Detecting faces in %s\n', test_scenes(random_number).name)
    img = imread( fullfile( test_scn_path, test_scenes(random_number).name ));
    ...
    for scale = scales
        ...
        scaled_image_ids = repmat({test_scenes(random_number).name}, size(index,1), 1);
